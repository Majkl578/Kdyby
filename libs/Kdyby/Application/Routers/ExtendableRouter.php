...
//		),
	 ...
	 */
	public function extend($name, $mask, array $metadata = array(), $strict = True)
	{
		if( $name === self::DADDY ){
		...
		return $this[$name] = new self($name, $mask, $metadata, $this->flags, $this, (bool)$strict);
	}
	...
	public function match(Nette\Web\IHttpRequest $httpRequest)
	{
		...
		if( $this->parent === Null ){ // daddy is first
			foreach ($this->routes as $rootRoute => $route) {
				if( $appRequest = $route->matchLeaf($uri, $httpQuery, (count($this)==0)) ){
					break;
				}
			}

			if( $appRequest === NULL ){ // childrens are bored
				return NULL;
			}

			// seems like daddy's children are having some work to do
			...
			if( $rootRoute !== array_shift($node['routeLeafs']) ){
				return Null;
			}

			$cursor = $this[$rootRoute];
			foreach( $node['routeLeafs'] AS $leaf ){
